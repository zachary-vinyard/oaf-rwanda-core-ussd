...
function rgo_prepayment_calc(client, input_menu_name){
    ...
    if(!new_client){
        ...
        var client_object = get_balance(JSON.parse(state.vars.client_json), lang);
        ...
        if(credit == 0 || balance == null || balance == undefined){
            // first take care of client name
            //var client_name = state.vars.client_name;
            //client.vars.name2 = client_name.split(" ", 1);
            //client.vars.name1 = client_name.split(" ", 2);
            //client.save();
        ...
        else{ // use returning client calc for clients that have 20A balance
            const req_perc = 0.6;
