...
const repayment_calc_options = {'RGO_Input_Table_20A' : rgo_prepayment_calc, 'Input_Table_20A' : core_prepayment_calc}

function rgo_prepayment_calc(account_number, input_menu_name){
...
function core_prepayment_calc(client_row, input_menu_name){ // here there be dragons
    //writeme
}

module.exports = function(account_number, input_menu_name, an_table, lang, max_chars){
    var get_client = require('./enr-retrieve-client-row');
    var client = get_client(account_number, an_table);
    ...
    var outstr = msgs('enr_prepayment', {'$PREP' : repayment_calc_options[input_menu_name](account_number, input_menu_name)}, lang)
    var outobj = {}
    var loc = 0;
    for(prod in products){
        console.log(prod)
