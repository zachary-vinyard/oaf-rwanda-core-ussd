...
function rgo_prepayment_calc(client, input_menu_name){
    ...
    if(new_client == null || new_client == undefined){
        ...
        console.log('Made it to arraylength calc');
        ...
        if(arrayLength > 1){
            ...
            console.log('Credit is ' + credit + " and balance is " + balance);
            if(credit > 0){
                state.vars.prev_client = true;
            }
            console.log('Made it past arraylength if statement' + state.vars.prev_client);
        }
    }
...
module.exports = function(account_number, input_menu_name, an_table, lang, max_chars){ //here there be tygers
    ...
    try{
        ...
        console.log('Prepayment is ' + prepayment + ' and input table is ' + input_table);
        if(state.vars.prev_client){
            pre_str = msgs('enr_repayment_alert', {'$REMAINDER' : prepayment}, lang) || ' ';
            console.log('repayment string is : "' + pre_str + ' "');
        }
        else{
