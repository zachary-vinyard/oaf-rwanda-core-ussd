...
module.exports = function(msg_name, opts, console_lang){ //todo: settings and translation tables should be generalized once this is online
    var output = '';
    var msg_lang = project.getOrCreateDataTable('ussd_settings').queryRows({'vars' : {'settings' : 'lang'}}).next().vars.value || 'en'; //this line needs work
    var newline_regex = RegExp(project.getOrCreateDataTable('ussd_settings').queryRows({'vars' :{'settings' : 'new_line'}}).next().vars.value) || '~B';
    opts = opts || {}; //default value for opts
    console_lang = console_lang || 'en'; //default value for console lang
    try{
        ...
        if(cursor.hasNext()){ //retrieve message from table
            ...
            if(keys.length > 0){ //insert options
                for(k in keys){
                    var regex = RegExp(k);
                    output = output.replace(regex, opts[k]); //output replace
                    con_msg = con_msg.replace(regex, opts[k]);  //console message replace
                }
            }
            console.log(con_msg);
        }
