...
module.exports = function(msg_name, opts){ //todo: settings and translation tables should be generalized once this is online
    try{
        var settings = project.getOrCreateDataTable('ussd_settings');
        var newline_regex = RegExp(settings.queryRows({'vars' :{'settings' : 'new_line'}}).next().vars.value);
        var msg_lang = settings.queryRows({'vars' : {'settings' : 'lang'}}).next().vars.value;
        ...
        admin_alert('USSD options table incomplete', 'Options table incomplete\nError: ' + error);
    }
