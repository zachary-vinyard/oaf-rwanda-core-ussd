...
module.exports = function(msg_name, opts, lang){ //todo: settings and translation tables should be generalized once this is online
    try{
        ...
        var msg_lang = lang || settings.queryRows({'vars' : {'settings' : 'lang'}}).next().vars.value;
    ...
    try{
        ...
        if(cursor.hasNext()){ //retrieve message from table
            ...
            var output = msg_row.vars[msg_lang].replace(RegExp(newline_regex, 'g'), "\n");
            var con_msg = msg_row.vars[console_lang].replace(RegExp(newline_regex, 'g'), "\n");
