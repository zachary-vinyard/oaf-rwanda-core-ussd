...
*/
 ...
 async function ask(){
    ...
    var session_cursor = await session_table.queryRows({
    ...
    if(session_cursor.hasNext()){
        ...
        var question_type = status.slice(0,(status.len)-1);
        var question_number = parseInt(status.slice((status.len)-1,status.len));
        ...
        if(diff < 15 ){
           ...
           // update various tracker variables and ask the next question
            state.vars.survey_type = status.slice(0,(status.len)-1);
            state.vars.step =  parseInt(status.slice((status.len)-1,status.len));
