...
module.exports = function(accnum, serial_no){
    ...
    // replace historic credit with 0 if undefined to preserve the calculation
    ...
    console.log('Month reg is ' + month_reg + ' and current month is ' + current_month + ' and types are ' + typeof(month_reg) + ' and ' + typeof(current_month));
    ...
    while(act_pointer.hasNext()){
        ...
        if(date_active.length < 24){
            ...
            month_active = new Date(date_active).getMonth();
            year_active = new Date(date_active).getYear();
        }
