...
module.exports = function(serial_no){
    ...
    if(state.vars.acc_empty){
        var serial_pointer = serial_table.queryRows({
            vars: {'serial_number' : serial_no, 'registered_account_number' : {exists : 0}}
        });
    }
    else{
        var serial_pointer = serial_table.queryRows({
            vars: {'serial_number' : serial_no, 'registered_account_number' : state.vars.account_number}
        });
    }
    ...
    // if the serial number shows up multiple times, send an alert
