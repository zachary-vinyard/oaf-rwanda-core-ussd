...
module.exports = function(account_number, client, chx_client_table_loc){ //client should be an object
    ...
    if(cursor.hasNext()){
        ...
        console.log('ordered chix ' + ordered_chickens)
        ...
        if(client_row.vars.chicken_only === 1){
            ...
            prep_check = Math.floor((paid - parseInt(client_row.vars.required_prepayment)) / 500);
        }
        //var prep_check = (parseInt(client_row.vars.chicken_only) === 1) ? ((paid / 500) >= ordered_chickens) : ((paid - parseInt(client_row.vars.required_prepayment)) / 500);
        console.log('prep check ' + prep_check)
