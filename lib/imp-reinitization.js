...
module.exports = function(){
    ...
    // if previous session was called by same user within previous 60 minutes and quiz has started, run reinitization process
    if(session_cursor.hasNext()){
        ...
        if(diff < 15 & row.vars.trn1){
            ...
            console.log(status.slice(1) + typeof(status.slice(1)));
            console.log(parseInt(status.slice(1)));
            state.vars.step = parseInt(status.slice(1)) + 1;
